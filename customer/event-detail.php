...
<div class="bg-white rounded-lg shadow overflow-hidden">
    ...
    <div class="p-8">
        <h1 class="text-3xl font-bold text-gray-800 mb-4"><?= htmlspecialchars($event['title']) ?></h1>
        ...
        <p class="text-gray-700 mb-6"><?= nl2br(htmlspecialchars($event['description'])) ?></p>

        <div class="border-t pt-6">
            <div class="flex justify-between items-center mb-4">
                <span class="text-2xl font-bold text-blue-600">$<?= number_format($event['price'], 2) ?> / សំបុត្រ</span>
                <?php if ($remaining > 0): ?>
                    <span class="text-sm text-green-600"><?= $remaining ?> សំបុត្រនៅសល់</span>
                <?php else: ?>
                    <span class="text-sm text-red-600 font-bold">សំបុត្រអស់ហើយ</span>
                <?php endif; ?>
            </div>
